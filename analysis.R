...
# Practice set info ---- 
#practice.begin("A2", learner="[Cecelia Thomas]", email="[ceceliat@uw.edu]")
...
attendees_distrubtion <- boxplot(num_attendees,
  main = "Distribution of Attendees",
  xlab = "Number of Protests",
  ylab = "Protests",
  horizontal = TRUE,
  notch = TRUE
)
...
log_attendees_distribution <- (log(num_attendees))
  boxplot(log_attendees_distribution,
  main = "Log of the Number of Attendees",
...
num_in_wa <- str_detect(locations, "WA")
num_in_wa <- sum(num_in_wa, na.rm = "TRUE")
print(num_in_wa)


# 3d: What proportion of protests occurred in Washington? (Variable: `prop_in_wa`)

prop_in_wa <- round(num_in_wa / num_protests * 100, 2)
print(prop_in_wa)
...
count_in_location <- function(location) {
  ...
  N <- str_detect(locations, location)
  N <- sum(N, na.rm = "TRUE")
  
 if (N != 0)
    print(paste("There were", N,"protests in", location))
 else
    print(paste("Location", location ,"not found"))
}
...
missing_summary = count_in_location("Redmond, WA")
...
in_2020 <- c(as.Date(str_subset(dates, "2020"))) 
#print(in_2020)


# 4f: Create a vector of the dates that are in 2019. (Variable: `in_2019`)

in_2019 <- c(as.Date(str_subset(dates, "2019")))
#print(in_2019)
...
count_on_date <- function(date){
  ...
  N <- str_detect(dates, date)
  N <- sum(N, na.rm = "TRUE")
  
  if (N != 0)
    print(paste("There were", N,"protests on", date))
}
...
change_july_protests <- by_month_table_2020[6] - by_month_table_2019[6]
...
#this function filters purpose and position_taken strings and creates a data frame of the matching protests
filter_positions <- function(purpose, position_taken = NULL){
  if(is.null(position_taken)){
    filter(protests, grepl(purpose, Event..legacy..see.tags.)) %>% head()
  }else {
    filter(protests, grepl(purpose, Event..legacy..see.tags.) & grepl(position_taken, Tags)) %>% head()
  }
}
...
#this function calls format_doc to create summary report and calls protest_df
filter_and_report <- function(purpose, position_taken=NULL){
  
  protest_df <- filter_positions(purpose, position_taken)
  format_doc(protest_df, purpose, position_taken)
...
#Limitations for these functions include case sensitivity as well as the inability to only have a value for position_taken 
#where if there is only a value for position_taken the function will not work
#if I had more time working on these functions I would have created more filters and better filters that got rid of any bugs to better sort the data
#I overall find these functions very useful as they filter the table pretty efficiently

#tests that filter_positions properly filters event and tag type without using entire phrase
filter_test1 <- filter_positions("Executive", "For")
View(filter_test1)

#tests function if purpose value doesn't exist
filter_test2 <- filter_positions("IronMan", "For")
View(filter_test2)

#tests if function will create data frame if position_taken = NULL
filter_test3 <- filter_positions("Civil Rights")
View(filter_test3)
...
write_report(protest_test, "C:/Users/cecel/Documents/info201/reports/report.md")


#tests if summary report works when purpose is empty
report_test1 <- filter_and_report("", "Against")
print(protest_test1)

#tests if summary report works when purpose and position_taken values exist
report_test2 <- filter_and_report("Environment", "for")
print(protest_test2)

#tests if summary report works when position_taken = NULL
report_test1 <- filter_and_report("Civil Rights", "")
print(protest_test)
